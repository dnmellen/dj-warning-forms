...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-warning-forms runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-warning-forms.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_warning_forms")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-warning-forms tests")
...
def release(c, bumpsize=''):
    ...
    import dj_warning_forms
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_warning_forms.__version__))
    c.run("git push --tags")
